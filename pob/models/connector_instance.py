...
class ConnectorInstance(models.Model):
    ...
    def refresh_list(self):
        # get_object_reference is not available in odoo15 changed it by _xmlid_lookup.
        # view_ref = self.env['ir.model.data'].get_object_reference('bridge_skeleton', 'connector_instance_form')
        view_ref = self.env['ir.model.data']._xmlid_lookup("%s.%s" % ('bridge_skeleton', 'connector_instance_form'))[1:3]
    ...
    def test_prestashop_connection(self):
        ...
        try:
            ...
            try:
                prestashop = PrestaShopWebServiceDict(url,key)
                _logger.info(f"The prestashop url and key is {prestashop}")
