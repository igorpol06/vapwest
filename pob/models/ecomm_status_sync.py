...
class ConnectorSnippet(models.TransientModel):
    ...
    def prestashop_after_order_invoice(self , connection, ecommerce_reference , id_order):
        return self.update_order_status_prestashop(connection, id_order, 2)
       
    @api.model
    def prestashop_after_order_cancel(self , connection, ecommerce_reference , id_order):
        return self.update_order_status_prestashop(connection, id_order, 6)
     
    @api.model
    def prestashop_after_order_shipment(self , connection, ecommerce_reference , id_order):
        return self.update_order_status_prestashop(connection, id_order, 4)

    def update_order_status_prestashop(self, connection, id_order, id_order_state):
        status = 'no'
        text = 'Status Successfully Updated'
        prestashop = connection.get('prestashop')
        try:
            ...
            if connection.get('notify',False):
                endpoint = 'order_histories?sendemail=1'
            else:
