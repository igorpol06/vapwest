...
class WkSkeleton(models.TransientModel):
    ...
    def delete_odoo_mappings(self, odoo_ids, odoo_model):
        ...
        try:
            ...
            mapModel = f'connector.{odoo_model}.mapping'
            mappings = self.env[mapModel].search(
                    [('odoo_id','in',odoo_ids),('instance_id','=',instance_id)])
            if mappings:
                mappings.unlink()
        except Exception as e:
            _logger.info("### Exception during odoo %s mapping delete :- %s",odoo_model,str(e))
            status = False
            msg = str(e)
        finally:
            return {
